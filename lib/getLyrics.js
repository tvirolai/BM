...
module.exports = function getLyrics(urls) {
  ...
  request(url, function (err, res, body) {
    if (!err) {
      ...
      band = band.substring(0, band.indexOf('LYRICS')).trim().replace(/\s/g, '_').replace(/\.|\'|\"|\:|#|\/|\(|\)|,/g, '').replace('&', 'and');
      var albumlyrics = {};
...
function saveToFile (band, title, lyric) {
  var songtitle = title.substring(title.indexOf('.') + 2).replace(/\s/g, '_').replace(/\.|\'|\"|\:|#|\/|\(|\)|,/g, '').replace('&', 'and');
